        ...
        if (request.ok) {
            ...
            request.json().then(function (response) {
                ...
                if (response.estado) {
                    ...
                    document.getElementById('titulo_promociones').textContent = 'Promociones';
                    // Se agregan las tarjetas a la etiqueta div mediante su id para mostrar los productos.
                    document.getElementById('promociones_contenido').innerHTML = content;
                } else {
                    // Se presenta un mensaje de error cuando no existen datos para mostrar.
                    document.getElementById('titulo_promociones').innerHTML = response.exception;
                }
            });
        } else {
...
function pagination() {
    ...
    fetch(API_PRODUCTOS + 'contadorProductosPromocion', {
        method: 'get'
    }).then(function (request) {
        // Se verifica si la petición es correcta, de lo contrario se muestra un mensaje en la consola indicando el problema.
        if (request.ok) {
...
function contadorPagination(navegar) {
    ...
    if (navegar == 1) {
        //Si no esta en la pagina 1, la flecha retroceder esta activa, eliminamos la clase desabilitado
