...
$pdf->startReport('Productos vendidos y monto recaudado en el día actual', 
'El reporte muestra los productos y las ganancias recaudadas en sus ventas durante un día');
// Se instancia el módelo de Productos para procesar los datos.
$productosdia = new Productos;
// Se verifica si existen registros (productos) para mostrar, de lo contrario se imprime un mensaje.
if ($dataProductosVD = $productosdia->openReporteProductosVD()) {
    ...
    //Establecemos formato horario de El Salvador
    date_default_timezone_set('America/El_Salvador');
    $dia_actual = date('d-m-Y');
    // Se imprimen las celdas con los encabezados.
    $pdf->cell(185, 10, utf8_decode('Fecha Actual: ' . $dia_actual), 1, 1, 'C', 1);
    ...
    $contador = 0;
    // Se recorren los registros ($dataProductosVD) fila por fila ($rowProducto).
    foreach ($dataProductosVD as $rowProducto) {
        if ($contador % 2 == 0) {
