...
if (isset($_GET['action'])) {
    ...
    switch ($_GET['action']) {
        ...
        case 'contadorProductos':
            ...
            break;
            //Caso de contador de promociónes
        case 'contadorProductosPromocion':
            if ($result['dataset'] = $productos->contadorProductosPromocion()) {
                $result['estado'] = 1;
            } elseif (Database::getException()) {
                $result['exception'] = Database::getException();
            } else {
            ...
            break;
            //Caso para las promociones
        case 'productoDescuentoLeer':
                if (!$productos->setContador($_POST['contador'])) {
                    $result['exception'] = 'Error al asignar el contador';
                } elseif ($result['dataset'] = $productos->productoDescuentoLeer()) {
                    $result['estado'] = 1;
                } elseif (Database::getException()) {
                    $result['exception'] = Database::getException();
                } else {
